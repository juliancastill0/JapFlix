...
const lista = document.querySelector('#lista'),
	...
	canvas_title = document.getElementById("offcanvasTopLabel"),
 	canvas_body = document.getElementById("offcanvas-body"),
	canvas_lista_more = document.getElementById("listaMore");
		...
		.then((data) => {
			data.forEach((pelicula) => {
				...
				li.addEventListener("click", () => {
					canvas_title.innerHTML = pelicula.title;
					canvas_body.innerHTML = `${pelicula.overview}<hr>`;
					for (let e of pelicula.genres){
						let generos = JSON.stringify(e["name"]);
						if (pelicula.genres.indexOf(e) === 2){
							canvas_body.innerHTML += generos.slice(1, -1);
						} else {
							canvas_body.innerHTML += generos.slice(1, -1) + " - ";
						};
					};
					let listaDatosPelicula = document.getElementsByClassName("lista-item");
					listaDatosPelicula[0].innerHTML = `<span>Year:</span> <span>${pelicula.release_date.split("-")[0]}</span>`;
					listaDatosPelicula[1].innerHTML = `<span>Runtime:</span> <span>${pelicula.runtime} mins</span>`;
					listaDatosPelicula[2].innerHTML = `<span>Budget:</span> <span>$${pelicula.budget}</span>`;
					listaDatosPelicula[3].innerHTML = `<span>Revenue:</span> <span>$${pelicula.revenue}</span>`;
				});
